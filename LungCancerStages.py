...
def runCNN():
    ...
    if os.path.exists("model/model_weights.keras") == False:  # Update to .keras extension
        model_check_point = ModelCheckpoint(filepath='model/model_weights.keras', verbose=1, save_best_only=True)  # Updated extension
        ...
        classifier = load_model("model/model_weights.keras")  # Updated extension
...
splitButton.place(x=10, y=150)
splitButton.config(font=font1)

trainButton = Button(main, text="Train CNN", command=runCNN)
trainButton.place(x=300, y=150)
...
closeButton.place(x=600, y=500)
closeButton.config(font=font1)

text = Text(main, height=20, width=130)
text.place(x=10, y=250)

main.config(bg="LavenderBlush")
main.mainloop()
